			...
			<div class="section">
				...
				<div class="slide page1" @touchstart="njEnd()">
					<div class="content">
						...
						<div class="p1-man" v-if="my.Sex == '男'">
						</div>
						<div class="p1-woman" v-else>
						</div>
						<div class="p1-ty">
						</div>
						<div class="p1-y1"></div>
						...
						<div class="mtext p1-text" v-if="!isPP">
							...
							<p>这是<span v-if="isHe && my.EmpName">{{ my.EmpName }}</span><template v-else>你</template>加入中原的日子，</p>
							<p>此刻，请用30秒去定格，</p>
							<p>那些属于中原的记忆。</p>
						</div>
						<div class="mtext p1-text" v-else>
							<p>在<span>{{ my.InCompanyDateStr }}</span> ，</p>
							<p><span v-if="isHe && my.EmpName">{{ my.EmpName | changeName }}</span><template v-else>你</template>成为了中原的一份子。
							</p>
						</div>
				...
				<div class="slide page2">
					...
					<div class="content">
						...
						<div class="p2-text">
							...
							<p><span v-if="isHe && my.EmpName">{{ my.EmpName | changeName }}</span><template v-else>你</template>的“星级”置业服务，
							</p>
							<p>从此启航...</p>
						</div>
						<div class="p2-phone">
					...
					<div class="content">
						...
						<div class="p3-text">
							<p>今年<span v-if="isHe && my.EmpName">{{ my.EmpName | changeName }}</span><template
									v-else>你</template>接待了<span>{{ my.CustomerCount }}个</span>线上客户，</p>
							<p>为五湖四海的人提供置业服务。</p>
							<p>因为<template v-if="isHe && my.EmpName"><template v-if="my.Sex == '男'">他</template><template
										v-else>她</template></template><template v-else>你</template>知道，</p>
							<p>每一次咨询背后，</p>
							<p>都是对美好生活的向往。</p>
						</div>
						<div class="p3-man">
				...
				<div class="slide page4" @click="lsPlay()" @touchmove="lsEnd()">
					...
					<div class="content">
						...
						<div class="p4-text">
							<p>2021年，</p>
							<template v-if="my.SalesCount > 0">
								<p><span v-if="isHe && my.EmpName">{{ my.EmpName | changeName }}</span><template v-else>你</template>和团队的小伙伴们，
								</p>
								<p>一起为<span v-if="my.SalesCount < 80">{{ my.SalesCount }}个</span><span v-else>超80个</span>房源找到新主人，</p>
							</template>
							<template v-else>
								<p><span v-if="isHe && my.EmpName">{{ my.EmpName | changeName }}</span><template v-else>你</template>和团队的小伙伴们，
								</p>
						...
						<div class="p4-lb"></div>
						<div class="goRight man peapo" :class="goFast ? 'fast' : 'goRight'" v-if="my.Sex == '男'">
						</div>
						<div class="goRight woman peapo" :class="goFast ? 'fast' : 'goRight'" v-else></div>
						<div class="p4-b"></div>
						<div class="p4-yu" @touchstart="gtouchstart()" @touchend="gtouchend()"></div>
					</div>
				</div>
				<div class="slide page5" @touchstart="lsEnd()">
					...
					<div class="content">
						...
						<div class="p5-y3"></div>
						<div class="p5-text">
							<template v-if="my.SalesAmount > 1000000">
								<p>说个了不起的数字，</p>
								<p><span v-if="isHe && my.EmpName">{{ my.EmpName | changeName }}</span><template v-else>你</template>和小伙伴们，</p>
								<p>共同促成了<span v-if="my.SalesAmount > 80000000">超8千万</span><span v-else>{{ my.SalesAmount |
									twoFixed }}万</span>的GMV，</p>
								<p>这绝对是一场硬仗，但<template v-if="isHe && my.EmpName"><template v-if="my.Sex == '男'">他</template><template
											v-else>她</template></template><template v-else>你</template>们做到了！</p>
							</template>
							<template v-else>
								<p>虽然今年房产市场行情不太好，</p>
								<p><span v-if="isHe && my.EmpName">{{ my.EmpName | changeName }}</span><template v-else>你</template>和小伙伴们,</p>
				...
				<div class="slide page6">
					...
					<div class="content">
						...
						<div class="p6-y3"></div>
						<div class="p6-text">
							<p><span>{{ my.TakeSeeDateStr }}这一天，</span></p>
							...
							<template v-if="my.TakeSeeEstate">
								...
								<p>看了个前前后后，</p>
								<p><template v-if="isHe && my.EmpName"><template v-if="my.Sex == '男'">他</template><template
											v-else>她</template></template><template v-else>你</template>和同事们真诚服务的汗水，</p>
								<p>浇灌深圳每一处的绿荫。</p>
							</template>
							<template v-else>
				...
				<div class="slide page7">
					...
					<div class="content">
						...
						<div class="p7-text">
							<p>关于买房有太多疑问，</p>
							<p>但只要有问题，就会得到回答！</p>
							<p v-if="my.BindCount > 36"><span>{{ my.BindCount }}</span>人因为<span v-if="isHe && my.EmpName">{{ my.EmpName |
								changeName }}</span><template v-else>你</template>的推荐，</p>
							<p v-else>很多人因为<span v-if="isHe && my.EmpName">{{ my.EmpName | changeName }}</span><template
									v-else>你</template>的推荐，</p>
							<p>下载<span>中原找房App</span>，</p>
							<p>房产交易路上不踩坑。</p>
						</div>
						<div class="p7-hb">
							<p>各大手机应用市场</p>
							<p>搜索<span>“中原找房”</span></p>
							<p>下载有惊喜</p>
						</div>
						<div class="p7-man">
				...
				<div class="slide page8">
					...
					<div class="content">
						...
						<div class="p8-y3"></div>
						<div class="p8-text">
							<p>在<span>{{ my.RegionName }}</span>，没有<span v-if="isHe && my.EmpName">{{ my.EmpName |
								changeName }}</span><template v-else>你</template>不熟悉的小区，</p>
							<p v-if="my.MainEstate">例如：<span v-if="my.MainEstate1">{{ my.MainEstate1 }}</span><span
									v-if="my.MainEstate2">，{{ my.MainEstate2 }}</span><span v-if="!my.MainEstate3"> ...</span><span
									v-else>，</span></p>
							<p v-if="my.MainEstate3"><span>{{ my.MainEstate3 }} ...</span></p>
							<p>话说，<template v-if="isHe && my.EmpName"><template v-if="my.Sex == '男'">他</template><template
										v-else>她</template> </template><template v-else>你</template>现在是行走的GPS。</p>
						</div>
						<div class="p8Map">
							<span class="ba"><img src="images/year/ba.png" width="100%" v-if="qy.indexOf('ba') < 0" /><template
									v-else><img src="images/year/ba1.png" width="100%" class="qyD" /><img src="images/year/cicle.png"
										class="cicle" /></template></span>
							<span class="gm"><img src="images/year/gm.png" width="100%" v-if="qy.indexOf('gm') < 0" /><template
									v-else><img src="images/year/gm1.png" width="100%" class="qyD" /><img src="images/year/cicle.png"
										class="cicle" /></template></span>
							<span class="ns"><img src="images/year/ns.png" width="100%" v-if="qy.indexOf('ns') < 0" /><template
									v-else><img src="images/year/ns1.png" width="100%" class="qyD" /><img src="images/year/cicle.png"
										class="cicle" /></template></span>
							<span class="lh"><img src="images/year/lh.png" width="100%" v-if="qy.indexOf('lhq') < 0" /><template
									v-else><img src="images/year/lh1.png" width="100%" class="qyD" /><img src="images/year/cicle.png"
										class="cicle" /></template></span>
							<span class="ft"><img src="images/year/ft.png" width="100%" v-if="qy.indexOf('ft') < 0" /><template
									v-else><img src="images/year/ft1.png" width="100%" class="qyD" /><img src="images/year/cicle.png"
										class="cicle" /></template></span>
							<span class="lg"><img src="images/year/lg.png" width="100%" v-if="qy.indexOf('lg') < 0" /><template
									v-else><img src="images/year/lg1.png" width="100%" class="qyD" /><img src="images/year/cicle.png"
										class="cicle" /></template></span>
							<span class="lhu"><img src="images/year/lhu.png" width="100%" v-if="qy.indexOf('lhu') < 0" /><template
									v-else><img src="images/year/lhu1.png" width="100%" class="qyD" /><img src="images/year/cicle.png"
										class="cicle" /></template></span>
							<span class="ps"><img src="images/year/ps.png" width="100%" v-if="qy.indexOf('ps') < 0" /><template
									v-else><img src="images/year/ps1.png" width="100%" class="qyD" /><img src="images/year/cicle.png"
										class="cicle" /></template></span>
							<span class="yt"><img src="images/year/yt.png" width="100%" v-if="qy.indexOf('yt') < 0" /><template
									v-else><img src="images/year/yt1.png" width="100%" class="qyD" /><img src="images/year/cicle.png"
										class="cicle" /></template></span>
							<span class="dp"><img src="images/year/dp.png" width="100%" v-if="qy.indexOf('dp') < 0" /><template
									v-else><img src="images/year/dp1.png" width="100%" class="qyD" /><img src="images/year/cicle.png"
										class="cicle" /></template></span>
						</div>
						<div class="p4-mtl"></div>
						<div class="p4-lb"></div>
				...
				<div class="slide page9">
					...
					<div class="content">
						...
						<div class="p9-text">
							<p>2021年，</p>
							<p><span v-if="isHe && my.EmpName">{{ my.EmpName | changeName }}</span><template
									v-else>你</template>最高一天走了<span>{{ my.StepNumber }}步</span>，</p>
							<p>全年共<span>{{ my.StepNumber * 360 }}步</span>。</p>
							<p>别不信！</p>
							<p>这足以绕深圳<span>{{ my.TurnsNumber }}圈</span>。</p>
						</div>
				...
				<div class="slide page10">
					...
					<div class="content">
						...
						<div class="p10-text">
							<p>线上咨询最晚的那个客户，还记得吗？</p>
							<template v-if="my.WebChatStr">
								<p><span>{{ my.WebChatStr }}，</span></p>
								<p><span v-if="isHe && my.EmpName">{{ my.EmpName | changeName }}</span><template v-else>你</template>还在线上贴心服务，
								</p>
								<p>为帮助客户实现安家梦而努力。</p>
							</template>
							<template v-else>
								<p>那天的月亮都睡着了，</p>
								<p><span v-if="isHe && my.EmpName">{{ my.EmpName | changeName }}</span><template
										v-else>你</template>还在为帮助客户实现安家梦而努力。</p>
							</template>
				...
				<div class="slide page11">
					...
					<div class="content">
						...
						<div class="p11-text">
							...
							<p>或许，一言“难”尽。</p>
							<p>但因为不放弃与竭诚的努力，</p>
							<p><span v-if="isHe && my.EmpName">{{ my.EmpName | changeName }}</span><template v-else>你</template>的燃力值，</p>
							<p>击败了中原<span>{{ my.EmpRateStr }}%</span>的经纪人。</p>
						</div>
				...
				<div class="slide page12">
					<div class="content">
						...
						<div class="p12-flag">
							...
							<label class="flg2" :class="flag == 2 ? 'on' : ''" @click="setFlag(2, '管住嘴迈开腿，腹肌一变八')">
								<span>管住嘴迈开腿，腹肌一变八</span>
							</label>
				...
				<div class="slide page13">
					<div class="content">
						<div class="p13Bj">
							<img src="images/year/hbBj.jpg" width="100%" id="scream" style="display: none;" crossOrigin='anonymous' />
							<img :src="tx" id="tx" style="display: none;" crossOrigin='anonymous' v-if="tx" />
							<vue-qr :text="downloadData.url" :margin="0" colorDark="#000" colorLight="#fff" :logoSrc="imageUrl"
								:logoScale="0.2" :size="220" ref="Qrcode" style="display: none;">
							</vue-qr>
...
var shareObj = { //微信链接分享
	...
	desc: '这里有你的2021大事记',
	link: shareLink,
	imgUrl: "https://sz.centanet.com/partner/house/shareImg/year3.jpg",
};
import Vue from 'vue';
...
export default {
	...
	methods: {
		...
		posterHb() {
			...
			ctx.fillStyle = "#fff";
			ctx.fillText(this.my.StoreName, 247, 1093);
			...
			this.$axios({
				...
				.then(res => {
					...
					if (res.data.Code == 101) {
						this.$toast.text("中原找房暂无头像，请先上传头像~");
						return;
						...
						// 	this.$toast.text("仅面向11月1日前入职的住宅部物业顾问同事");
						// 	return
						// }
		...
		start() {
			setTimeout(() => {
				...
				// var audio1 = document.getElementById("music1"); //鸟叫 猫叫
				if (audio.paused) {
					audio.play();
				}
				// if(audio1.paused){
...
.container {
	...
	.load {
		...
		left: 0;
		z-index: 2002;
		background: url(../images/year/loadBj.jpg?v=3) center bottom no-repeat;
	...
	.go-r {
		background: url(../images/year/go-r.png) center bottom no-repeat;
		background-size: 100%;
		width: 0.51rem;
		height: 0.55rem;
		position: absolute;
	...
	.page2 {
		...
		.p2-phone {
			...
			.p2-msg {
				width: 2.29rem;
				height: 4.18rem;
				background: url(../images/year/p2-msg.png?v=1) center bottom no-repeat;
				background-size: 100%;
				display: none;
				// position: absolute;
				// bottom: 0.2rem;
			...
			.p2-msg1 {
				width: 2.29rem;
				height: 4.04rem;
				background: url(../images/year/p2-msg1.png?v=1) center bottom no-repeat;
				background-size: 100%;
				// position: absolute;
				// bottom: 0.2rem;
				// right: 0.25rem;
	...
	.page4 {
		...
		.p4-b {
			width: 1.39rem;
			height: 0.57rem;
			background: url(../images/year/p4-br.png) left center no-repeat;
	...
	.page7 {
		background: url(../images/year/p7Bj2.jpg) center bottom no-repeat;
		background-size: cover;
		...
		.p7-hb {
			width: 2.1rem;
			height: 1.8rem;
			background: url(../images/year/p7-hb.png) center bottom no-repeat;
			background-size: 100%;
			position: absolute;
			right: 0.2rem;
			bottom: 2rem;
			font-size: 0.2rem;
			padding: 0.5rem 0.11rem 0.35rem;
			color: #fff;
...
#logo {
	...
	top: 0.4rem;
	z-index: 6;
	background: url(../images/year/y-logo.png) center bottom no-repeat;
	background-size: 100%;
}</style>
